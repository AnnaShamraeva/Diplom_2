...
class TestBurger:
    def test_set_buns_correct(self, mock_bun):
        burger = Burger()
        burger.set_buns(mock_bun)
        assert burger.bun == mock_bun

    def test_add_ingredient_success(self, mock_ingredient_1):
        burger = Burger()
    ...
    def test_move_ingredient_success(self, burger):
        ...
        assert burger.ingredients[0] == ingredient_2 # проверить порядок
        assert burger.ingredients[1] == ingredient_1

    def test_get_price_correct(self, mock_bun, mock_ingredient_1, mock_ingredient_2):
        burger = Burger()
        burger.set_buns(mock_bun)
        burger.add_ingredient(mock_ingredient_1)
        burger.add_ingredient(mock_ingredient_2)
        mock_bun.get_price.return_value = 50            # price = self.bun.get_price() * 2
        mock_ingredient_1.get_price.return_value = 100  # price += ingredient.get_price()
        mock_ingredient_2.get_price.return_value = 150
        assert burger.get_price() == 350

    def test_get_receipt_correct(self, mock_bun, mock_ingredient_1, mock_ingredient_2):
        burger = Burger()
        burger.set_buns(mock_bun)
        burger.add_ingredient(mock_ingredient_1)
        burger.add_ingredient(mock_ingredient_2)
